...
bool useLoadedSkyTextures = true;
bool useLoadedEnvTextures = true;
...
float maxCameraDisplacementBeforeMovingRenderWindow = 30.0f;
bool renderWindow = true;
float renderRadius = 75.0f;
...
void CHRecurse(std::pair<int,int> c1, std::pair<int,int> c2, std::pair<int,int> c3, std::pair<int,int> c4, int w, float range) {
  if(w < 1)
    return;

  std::pair<int,int> mid = std::make_pair(c1.first + w/2, c1.second + w/2);
  float avg = map[c1.first][c1.second] + map[c2.first][c2.second] + map[c3.first][c3.second] + map[c4.first][c4.second];
  avg /= 4;
  map[mid.first][mid.second] = avg + randRange(-range/2, range/2);

  std::pair<int,int> e1 = std::make_pair(c1.first, mid.second);
  avg = map[c1.first][c1.second] + map[c2.first][c2.second] + map[mid.first][mid.second];
  if(e1.first - w/2 >= 0 && map[e1.first - w/2][e1.second] != -1) {
  ...
  std::pair<int,int> e2 = std::make_pair(mid.first, c2.second);
  avg = map[c2.first][c2.second] + map[c3.first][c3.second] + map[mid.first][mid.second];
  if(e2.second + w/2 < mapSize && map[e2.first][e2.second + w/2] != -1) {
  ...
  std::pair<int,int> e3 = std::make_pair(c3.first, mid.second);
  avg = map[c3.first][c3.second] + map[c4.first][c4.second] + map[mid.first][mid.second];
  if(e3.first + w/2 < mapSize && map[e3.first + w/2][e3.second] != -1) {
  ...
  std::pair<int,int> e4 = std::make_pair(mid.first, c4.second);
  avg = map[c4.first][c4.second] + map[c1.first][c1.second] + map[mid.first][mid.second];
  if(e4.second - w/2 >= 0 && map[e4.first][e4.second - w/2] != -1) {
  ...
  float r = std::pow(2.0, -smoothness)*range;
  CHRecurse(c1, e1, mid, e4, w/2, r);
  CHRecurse(e1, c2, e2, mid, w/2, r);
  CHRecurse(mid, e2, c3, e3, w/2, r);
  CHRecurse(e4, mid, e3, c4, w/2, r);
}
...
GLuint generateSky(std::vector<Image>& images) {
  ...
  if(!useLoadedSkyTextures) {
    // Set up the texture byte array
    int width = 600;
    int height = 600;
    bytes = new unsigned char[width * height];
    unsigned char* currow = &bytes[0];
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        float nx = ((float) i)/((float) height);
        float ny = ((float) j)/((float) width);
        currow[j] = 128*PerlinNoise(100*nx, 100*ny) + 64*PerlinNoise(200*nx, 200*ny) + 127;
      }
      currow += width;
    }

    top = new unsigned char[w*h*3];
    left = new unsigned char[w*h*3];
    front = new unsigned char[w*h*3];
    right = new unsigned char[w*h*3];
    back = new unsigned char[w*h*3];
    bottom = new unsigned char[w*h*3];

    currow = &top[0];
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[i*width + (100+j)];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[(100 + i)*width + j];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[(100 + i)*width + (100+j)];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[(100 + i)*width + (200+j)];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[(100 + i)*width + (300+j)];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        currow[3*j] = bytes[(200 + i)*width + (100+j)];
        currow[3*j + 1] = 255;
        currow[3*j + 2] = 255;
      }
      currow += w * 3;
  ...
  if (useLoadedSkyTextures) {
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, right);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, left);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, top);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, bottom);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, front);
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, back);
  }
...
int main(int argc, char* argv[]) {
  ...
  grass.width = 600;
  grass.height = 600;
  grass.bytes = new unsigned char[grass.width * grass.height * 3];
  unsigned char* currow = &grass.bytes[0];
  for (int i = 0; i < grass.height; i++) {
    for (int j = 0; j < grass.width; j++) {
      currow[j*3] = 0;
      //currow[j*3 + 1] = 255*(std::abs(PerlinNoise(10*i, 10*j)) + 0.75*std::abs(PerlinNoise(5*i, 5*j)) + 0.5*std::abs(PerlinNoise(1*i, 1*j)));
      currow[j*3 + 1] = 128*PerlinNoise(100*i, 100*j) + 64*PerlinNoise(200*i, 200*j) + 127;
      currow[j*3 + 2] = 0;
    }
    currow += grass.width * 3;
  }
  ...
  mountain.width = 600;
  mountain.height = 600;
  mountain.bytes = new unsigned char[mountain.width * mountain.height * 3];
  currow = &mountain.bytes[0];
  for (int i = 0; i < mountain.height; i++) {
    for (int j = 0; j < mountain.width; j++) {
      int value = 128*(std::abs(PerlinNoise(10*i, 10*j)) + 0.75*std::abs(PerlinNoise(20*i, 20*j)) + 10*std::abs(PerlinNoise(40*i, 40*j)));
      currow[j*3] = value;
      currow[j*3 + 1] = value;
      currow[j*3 + 2] = value;
    }
    currow += mountain.width * 3;
  }
  ...
  if (useLoadedEnvTextures) {
    ...
    CHECK_GL_ERROR(glBindTexture(GL_TEXTURE_2D, grasstex));
    CHECK_GL_ERROR(glTexStorage2D(GL_TEXTURE_2D,  1, GL_RGB8, grass.width, grass.height));
    CHECK_GL_ERROR(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, grass.width, grass.height, GL_RGB, GL_UNSIGNED_BYTE, (const GLvoid*) grass.bytes));
  }
  ...
  if (useLoadedEnvTextures) {
    ...
    CHECK_GL_ERROR(glBindTexture(GL_TEXTURE_2D, mountaintex));
    CHECK_GL_ERROR(glTexStorage2D(GL_TEXTURE_2D,  1, GL_RGB8, mountain.width, mountain.height));
    CHECK_GL_ERROR(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mountain.width, mountain.height, GL_RGB, GL_UNSIGNED_BYTE, (const GLvoid*) mountain.bytes));
  }
